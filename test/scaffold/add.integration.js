...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/bitcore-node-zula.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update bitcore-node-zula.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'bitcore-lib-zula': '^v0.13.17',
          'bitcore-node-zula': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/bitcore-node-zula.json');
