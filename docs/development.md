...
To develop bitcore-node-zula:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-zula.git
...
We now will setup symlinks in `bitcore-node-zula` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-zula
ln -s ~/bitcoind-rpc-zula
```
...
cd devnode
mkdir node_modules
touch bitcore-node-zula.json
touch package.json
```

Edit `bitcore-node-zula.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-zula/bin/bitcore-node-zula start
```